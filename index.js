...
function cumlelereDonustur(dizi, ayrac ){
	let yenicumle = dizi.map(cumle => {return cumle.join(ayrac)} )
	return yenicumle
}
...
function paragrafOlustur(dizi, callbackKur, callbackDonustur){
	let yeni = callbackDonustur(dizi, " ")
	return callbackKur(yeni[1], yeni[3], yeni[5], yeni[7], yeni[9])
}
...
function emojileriDonustur(mesaj, emoji){
	let key = Object.keys(emoji);
	let value = Object.values(emoji);
	for(let i=0; i < emoji.length; i++){
		mesaj.replaceAll(key[i], value[i])
	}
	return mesaj
}
